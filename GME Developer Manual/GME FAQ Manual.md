...
#### 请问GME支持哪些游戏引擎及平台？
GME 目前支持的游戏引擎有Unity引擎，Unreal引擎及Cocos2d引擎，支持的平台包括Windows平台、Mac平台、iOS平台及Android平台。
...
请根据使用的场景，在控制台选择相应的采样率（16k或者48k），在开发接入中可以通过接口调整音频类型，但采样率不超过控制台所选择的采样率。例如，在控制台选择了16k的采样率，如果客户端调整为高清音质，采样率依旧为16k。如果控制台选择的48k的采样率，如果客户端调整为流畅音质，采样率即为16k。
...
#### 调用poll的方法的时间间隔多少比较合适  现在我看demo里面是1/30？
...
#### 询问有没有调用 EnableMic 函数前调用的获取麦克风好坏状态的接口？
...
#### GME的实时语音小队语音如何接入？小队语音是否有距离衰减？
实时语音小队语音接入请参考[小队语音文档](https://cloud.tencent.com/document/product/607/17972)。实现小队语音功能后，同小队语音无距离衰减，全局语音有距离衰减，衰减系数参考文档。

#### 请问为了实现3D音效，用户使用的麦克风和扬声器有什么要求么？
为了实现3D音效需要播放端支持双声道。

#### GME的实时语音3D音效如何接入？
实时语音3D音效接入请参考[3D音效文档](https://cloud.tencent.com/document/product/607/18218)。

#### 为什么下载下来的 SDK 文档和 Demo 里都没有 Authbuffer 文件？
Authbuffer 此文件已经合并了，请在 SDK 里面全局搜索一下。

#### tea加密有没有lib文件？
我们提供[Authbuffer编译文档及zip包](https://github.com/TencentMediaLab/GME/blob/master/GME%20Developer%20Manual/GME%20Authbuffer%20Manual.md)。
...
## 语音消息及转文本服务常见问题
#### GME的语音消息及转文本服务支持什么语音？
目前在腾讯云控制台开通的语音消息及转文本服务仅支持中文，若转文本服务需支持其他外文，请咨询商务。

#### GME的语音消息及转文本服务可以与实时语音一起使用吗？
可以的，调用相关接口进行切换就行。

### GME的语音消息及转文本服务一直返回 OK ，但功能是不可用的。
GME的语音消息及转文本服务需要导出移动端测试。


#### 调用上传语音文件接口上传成功后，返回值中的 fileid 是个 url，该怎么使用？
调用下载文件接口然后用这个url去下载文件。
