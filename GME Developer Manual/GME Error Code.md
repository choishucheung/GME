...
|AV_ERR_ENDPOINT_NOT_EXIST     		|1401|AVEndpoint对象不存在                                       	 							|当成员没有在发语音或视频文件时，去获取它的AVEndpoint对象时，就可能产生这个错误。                                                                                                                                                          															|修改业务侧逻辑，确保调用SDK接口的时机的正确性。                                         			 													|
...
|QAVPTTERROR_RECORDER_INIT_ERROR 						|4098|录音错误 |初始化错误|分析日志，获取后台返回给客户端的真正错误码，并联系后台同事协助解决。                       							|
...
|QAVPTTERROR_RECORDER_NO_AUDIO_DATA_WARN               |4101|录音错误 |没有采集到音频数据|分析日志，获取后台返回给客户端的真正错误码，并联系后台同事协助解决。       						|
